...
void PartialPlayer_ActionAbility_Combination::getMove(GameState & state, Move & move)
{
...
PPPtr PartialPlayer_ActionAbility_Combination::clone() 
{ 
    ...
    PPPtr combo(new PartialPlayer_ActionAbility_Combination(_playerID, newCombo));
    combo->setDescription(_description);
    return combo;
}

void PartialPlayer_ActionAbility_Combination::setBuyLimits(const BuyLimits & buyLimits)
{
...
std::string PartialPlayer_ActionAbility_Combination::getDescription(size_t depth) 
{ 
    std::stringstream ss; 
